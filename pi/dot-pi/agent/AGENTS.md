...
- The client Mac is `nathan-dev-client` on Tailscale (`100.68.116.104`). When Nathan types into ag's Herdr from it, the keystrokes go through the client first, so a broken shortcut may be a client-side problem. To reach the client from ag, use `ssh nathan-dev-client`. If SSH is refused, ask Nathan to run on the client: `sudo systemsetup -setremotelogin on`, then append ag's `~/.ssh/id_ed25519.pub` to the client's `~/.ssh/authorized_keys` (dir `700`, file `600`), or `tailscale up --ssh`. The client user is `nathan` unless he says otherwise.
