...
function handleGlobalAppEvent(name, event, app)
	...
	elseif event == hs.application.watcher.deactivated then
		if watchers[app:pid()] then
			watchers[app:pid()]:stop()
		end
	end
end
...
module.eventwatcher1 = hs.eventtap.new(
	{ hs.eventtap.event.types.keyUp, hs.eventtap.event.types.keyDown, hs.eventtap.event.types.flagsChanged },
	function(e)
		if not shiftUpToParensEnabled then
			return
		end
		...
		if shiftState == "up" and not (flags.alt or flags.cmd or flags.ctrl or flags.fn) then
			...
			if
				not module.cancelShiftModification
