...
module.hs_help = lrhk:bind({ "rCmd", "rAlt" }, "h", function()
	if
		doc.hsdocs._browser
		and doc.hsdocs._browser:hswindow()
		and doc.hsdocs._browser:hswindow() == window.frontmostWindow()
	then
		...
		if doc.hsdocs._browser then
			...
			timer.waitUntil(function()
				return doc.hsdocs._browser:hswindow()
			end, function(t)
				doc.hsdocs._browser:hswindow():focus()
			end, 0.1)
		else
			doc.hsdocs.help()
		end
